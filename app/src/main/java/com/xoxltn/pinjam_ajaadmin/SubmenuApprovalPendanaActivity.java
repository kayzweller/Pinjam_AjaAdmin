...
public class SubmenuApprovalPendanaActivity extends AppCompatActivity {

    private DocumentReference docRef;
    private DocumentReference loanRef;

    private String mIDPendana, mDocPath, mDocName, mNominalPinjam, mSet;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_submenu_approval_pendana);

        // calling extra
        Bundle extras = this.getIntent().getExtras();
        if (extras != null) {
            mIDPendana = extras.getString(SubmenuApprovalActivity.SEND_ID_PENDANA);
            mDocPath = extras.getString(SubmenuApprovalActivity.SEND_DOCUMENT_PATH);
            mDocName = extras.getString(SubmenuApprovalActivity.SEND_DOCUMENT_DOC);
            mNominalPinjam = extras.getString(SubmenuApprovalActivity.SEND_LEND_NOMINAL);
        }

        Log.e("mNominalPinjam", mNominalPinjam);

        FirebaseFirestore fire = FirebaseFirestore.getInstance();
        docRef = fire.collection("USER").document(mIDPendana);
        loanRef = fire.document(mDocPath);
    ...
    private void callDataPendana(final String IDPendana) {
        ...
        fireStorage.getDownloadUrl().addOnSuccessListener(uri -> Picasso.get().load(uri).fit()
                .centerCrop().into(imageview_ktp));
    ...
    public void onClickPerbaikiData (View view) {
        loanRef.update("id_pendana", null);
        loanRef.update("pendanaan_status", false);
        loanRef.update("pendanaan_req", true);
        loanRef.update("pinjaman_status", "MENUNGGU PENDANAAN");
        ...
        docRef.update("info_rekening", "un-done")
                .addOnSuccessListener(aVoid -> {
                    Toast.makeText(SubmenuApprovalPendanaActivity.this,
                            "Request pendanaan dibatalkan!", Toast.LENGTH_SHORT).show();
                    startActivity(new Intent(SubmenuApprovalPendanaActivity.this,
                            MainDashboardActivity.class));

                    // call the borrowing nominal
                    switch (mNominalPinjam) {
                        case "500000":
                            mSet = "Rp. 500.000,-";
                            break;
                        case "1000000":
                    ...
                    dataNotif.put("id_user", mIDPendana);
                    dataNotif.put("notif_type", true);
                    dataNotif.put("notif_date", mCurrentDate);
                    dataNotif.put("notif_title", "Pendanaan Pinjaman Dibatalkan!");
                    dataNotif.put("notif_detail", "Maaf pendanaan pinjaman Anda dengan ID : " +
                            mDocName + " dengan nilai pinjaman " + mSet + " ditolak. Jika" +
                            " Anda sudah mentransfer untuk pendanaan, uang Anda akan" +
                            " ditransfer kembali ke rekening Anda.");
